...
class TextMelDECADataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.trainset = TextMelDECADataset(
            self.hparams.name,
            self.hparams.train_filelist_path,
            self.hparams.n_spks,
            self.hparams.cleaners,
            #self.hparams.motion_folder,
            self.hparams.expression_folder,
            self.hparams.pose_folder,
        ...
        self.validset = TextMelDECADataset(
            self.hparams.name,
...
class TextMelDECADataset(torch.utils.data.Dataset):
    def __init__(self, name, filelist_path, n_spks, cleaners, #motion_folder,
                 expression_folder, pose_folder, add_blank=True,
        ...
        self.expression_fileloc = Path(expression_folder)
        self.pose_fileloc = Path(pose_folder)
        ...
        if data_parameters is not None:
            ...
            self.data_parameters = { 'mel_mean': 0, 'mel_std': 1, 'expression_mean': 0, 'expression_std': 1 ,'pose_mean': 0, 'pose_std': 1 }
        random.seed(seed)
    ...
    def get_pair(self, filepath_and_text):
        ...
        expression = self.get_expression(filepath, mel.shape[1])
        pose = self.get_pose(filepath, mel.shape[1])
    ...
    def get_expression(self, filename, mel_shape, ext=".decaexp.pkl"):
        file_loc = self.expression_fileloc / Path(Path(filename).name).with_suffix(ext)
        expression = torch.from_numpy(pd.read_pickle(file_loc).to_numpy())
        expression = F.interpolate(expression.T.unsqueeze(0), mel_shape).squeeze(0)
        expression = normalize(expression, self.data_parameters['expression_mean'], self.data_parameters['expression_std'])
        return expression

    def get_pose(self, filename, mel_shape, ext=".decapose.pkl"):
        file_loc = self.pose_fileloc / Path(Path(filename).name).with_suffix(ext)
        pose = torch.from_numpy(pd.read_pickle(file_loc).to_numpy())
        pose = F.interpolate(pose.T.unsqueeze(0), mel_shape).squeeze(0)
        pose = normalize(pose, self.data_parameters['pose_mean'], self.data_parameters['pose_std'])
        return pose
...
class TextMelMotionBatchCollate:
    ...
    def __call__(self, batch):
        ...
        n_expression = batch[0]['y_expression'].shape[-2]
        n_pose = batch[0]['y_pose'].shape[-2]
        
        y = torch.zeros((B, n_feats, y_max_length), dtype=torch.float32)
        x = torch.zeros((B, x_max_length), dtype=torch.long)
        #y_motion = torch.zeros((B, n_motion, y_max_length), dtype=torch.float32)
        y_expression = torch.zeros((B, n_expression, y_max_length), dtype=torch.float32)
        y_pose = torch.zeros((B, n_pose, y_max_length), dtype=torch.float32)
        y_lengths, x_lengths = [], []
        texts, spks = [], []

        for i, item in enumerate(batch):
            #y_, x_, y_motion_ = item['y'], item['x'], item['y_motion']
            y_, x_, y_expression_, y_pose_ = item['y'], item['x'], item['y_expression'], item['y_pose']
            y_lengths.append(y_.shape[-1])
            x_lengths.append(x_.shape[-1])
            y[i, :, :y_.shape[-1]] = y_
            x[i, :x_.shape[-1]] = x_
            #y_motion[i, :, :y_motion_.shape[-1]] = y_motion_
            y_expression[i, :, :y_expression_.shape[-1]] = y_expression_
            y_pose[i, :, :y_pose_.shape[-1]] = y_pose_
