...
class BaseLightningClassDECA(LightningModule, ABC):
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        total_loss = loss_dict["dur_loss"] + loss_dict["diff_loss"] #sum(loss_dict.values())
        self.log(
            "loss/train",
            total_loss,
            on_step=True,
    ...
    def on_validation_end(self) -> None:
        if self.trainer.is_global_zero:
            ...
            for i in range(2):
                ...
                self.logger.experiment.add_image(f'generated_enc/expression_{i}', plot_tensor(y_expression_enc.squeeze().cpu()), self.current_epoch, dataformats='HWC')
                self.logger.experiment.add_image(f'generated_dec/expression_{i}', plot_tensor(y_expression_dec.squeeze().cpu()), self.current_epoch, dataformats='HWC')
                self.logger.experiment.add_image(f'generated_enc/pose_{i}', plot_tensor(y_pose_enc.squeeze().cpu()), self.current_epoch, dataformats='HWC')
                self.logger.experiment.add_image(f'generated_dec/pose_{i}', plot_tensor(y_pose_dec.squeeze().cpu()), self.current_epoch, dataformats='HWC')
                """
                """
                self.logger.experiment.log({
                ...
                #self.logger.experiment.add_image(f'generated_dec/motion_{i}', plot_tensor(y_motion_dec.squeeze().cpu()), self.current_epoch, dataformats='HWC')
                """
                self.logger.experiment.log({f"generated_enc/expression_{i}": wandb.Image(plot_tensor(y_expression_enc.squeeze().cpu()), caption=f"Epoch {self.current_epoch}"), "epoch": self.current_epoch})
                self.logger.experiment.log({f"generated_dec/expression_{i}": wandb.Image(plot_tensor(y_expression_dec.squeeze().cpu()), caption=f"Epoch {self.current_epoch}"), "epoch": self.current_epoch})
                self.logger.experiment.log({f"generated_enc/pose_{i}": wandb.Image(plot_tensor(y_pose_enc.squeeze().cpu()), caption=f"Epoch {self.current_epoch}"), "epoch": self.current_epoch})
                self.logger.experiment.log({f"generated_dec/pose_{i}": wandb.Image(plot_tensor(y_pose_dec.squeeze().cpu()), caption=f"Epoch {self.current_epoch}"), "epoch": self.current_epoch})
                """
